...
Der Loop Diskriminator ist durch ein einfaches \emph{P-Glied} mit Verstärkungsfaktor $K_d$ modelliert.
...
\begin{align}
    b_0 &= \frac{16 ~B_L ~\zeta^2\cdot(4~\zeta^2+2 ~B_L~ T + 1)}{K_0 ~K_d \cdot (4 ~\zeta^2+1)^2} \\
    b_1 &= \frac{-16 ~B_L ~\zeta^2\cdot(4~\zeta^2-2 ~B_L~ T + 1)}{K_0 ~K_d \cdot (4 ~\zeta^2+1)^2}\\
    a_0 &= 1 \\
    a_1 &= -1
\end{align}
