...
\FG{\input{figures/GPSConstellation.tex}}{Konstellationsdiagramm des L1 GPS Signals (blau, $P_{0,1}+C_{0,1}$). Das GPS Signals ergibt sich durch Addition des modulierten \gls{CA} Trägers (rot, $C_{0,1}$) mit dem modulierten, um \SI{3}{\dB} gedämpften, \gls{PY} Trägers (grün, $C_{0,1}$).}{GPSConstellation}
...
\FGimg{trilateration.png}{Bestimmung der Position mittels Lateration. Wenn der Beobachter die Entfernung zu mindestens drei Punkten mit bekannten Koordinaten messen kann, so er seine eigenen Koordinaten errechnen.}{0.8\textwidth}