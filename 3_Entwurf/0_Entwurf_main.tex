...
Auf der FPGA Seite müssen die Daten gepuffert werden, da der DSP nicht immer sofort bereit ist die Daten zu empfangen. Dazu werden die Daten in eine \gls{FIFO} Warteschlange geschrieben. (\FR{GPS_UML_Rohdatenpuffer.pdf}). Zur DSP Seite hin ist der FIFO durch weitere Komponenten als eine Speicherzelle abstrahiert, sodass der DSP über die EBIU Schnittstelle auf den FIFO zugreifen kann. 
...
\FGimg[Rohdatenschnittstelle auf Hardware Ebene]{GPS_UML_Rohdatenpuffer.pdf}{Rohdatenschnittstelle auf Hardware Ebene. Die Rohdaten werden in einen FIFO geschrieben. Der DSP kann auf den FIFO über das EBIU Interface wie auf einen Speicher zugreifen.}{0.7\textwidth}
